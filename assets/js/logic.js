...
function checku(u) {
  ...
  if (
    !bot[u] &&
    names[u + "_player_name"] != null &&
    names[u + "_player_name"] != ""
  ) {
    ret = true;
  }
...
var turn_oder_lobby = [
  {
    group: "RED",
    rank: 0,
    players: [red_player_1, red_player_2, red_player_3, red_player_4],
  },
  {
    group: "GREEN",
    rank: 0,
    players: [green_player_1, green_player_2, green_player_3, green_player_4],
  },
  ...
  },
  {
    group: "BLUE",
    rank: 0,
    players: [blue_player_1, blue_player_2, blue_player_3, blue_player_4],
  },
];
...
function init() {
  init_r = true;
  if (checku("red")) {
    $(".red-home .white-box .player-room").append(
      red_player_1.player +
        red_player_2.player +
        red_player_3.player +
        red_player_4.player
  ...
  if (checku("green")) {
    $(".green-home .white-box .player-room").append(
      green_player_1.player +
        green_player_2.player +
        green_player_3.player +
        green_player_4.player
    );
    turn_oder.push(turn_oder_lobby[1]);
  }
  if (checku("yellow")) {
    $(".yellow-home .white-box .player-room").append(
      yellow_player_1.player +
        yellow_player_2.player +
        yellow_player_3.player +
        yellow_player_4.player
    );
    turn_oder.push(turn_oder_lobby[2]);
  }
  if (checku("blue")) {
    $(".blue-home .white-box .player-room").append(
      blue_player_1.player +
        blue_player_2.player +
        blue_player_3.player +
        blue_player_4.player
    );
...
$("#dice").click(function () {
  ...
  if (getActivePlayers(current_turn.players[0].home) == 0 && dice_value != 6) {
    console.log(
      current_turn.group +
        " : no player is active , shifting turn to next player"
    );
    hold(hold_time).then(() => {
      updateTurn(++current_turn_index);
      activateDice();
    ...
    !getHomePlayers(current_turn.players[0].home) &&
    getActivePlayers(current_turn.players[0].home) == 0 &&
    dice_value == 6
  ) {
    console.log(
      current_turn.group +
        " : you can't use this dice value, so shifting turn to next player"
    );
    hold(hold_time).then(() => {
      updateTurn(++current_turn_index);
      activateDice();
    ...
    getActivePlayers(current_turn.players[0].home) == 1 &&
    dice_value < 6
  ) {
    console.log(
      current_turn.group +
        " : you only have 1 active player so moving automatically"
    );
    ...
    getActivePlayers(current_turn.players[0].home) == 1 &&
    getHomePlayers(current_turn.players[0].home) == 0 &&
    dice_value == 6
  ) {
    console.log(
      current_turn.group +
    ...
    getHomePlayers(current_turn.players[0].home) == 1 &&
    getActivePlayers(current_turn.players[0].home) == 0 &&
    dice_value == 6
  ) {
    console.log(
      current_turn.group +
        " : you got 6 but , you don't have any active players and only 1 player in home so making it active"
    );
    hold(hold_time).then(() => {
      for (i = 0; i < 4; i++) {
...
function updateTurn(cti) {
  ...
  if (current_turn.group == "YELLOW") $(".yellow_control").show();
  // inout_sound.play();
  $(current_turn.players[0].home + " .white-box").css(
    "transform",
    "scale(1.15)"
  );
  six_count = 0;
...
function getActivePlayers(home) {
  ...
  if (home == ".red-home") {
    if (
      red_player_1.status == "active" &&
      red_player_1.current_position + dice_value <= red_player_1.path.length
    )
      active_players++;
    if (
      red_player_2.status == "active" &&
      red_player_2.current_position + dice_value <= red_player_2.path.length
    )
      active_players++;
    if (
      red_player_3.status == "active" &&
      red_player_3.current_position + dice_value <= red_player_3.path.length
    )
      active_players++;
    if (
      red_player_4.status == "active" &&
      red_player_4.current_position + dice_value <= red_player_4.path.length
    )
      active_players++;
  }
  if (home == ".green-home") {
    if (
      green_player_1.status == "active" &&
      green_player_1.current_position + dice_value <= green_player_1.path.length
    )
      active_players++;
    if (
      green_player_2.status == "active" &&
      green_player_2.current_position + dice_value <= green_player_2.path.length
    )
      active_players++;
    if (
      green_player_3.status == "active" &&
      green_player_3.current_position + dice_value <= green_player_3.path.length
    )
      active_players++;
    if (
      green_player_4.status == "active" &&
      green_player_4.current_position + dice_value <= green_player_4.path.length
    )
      active_players++;
  }
  if (home == ".yellow-home") {
    if (
      yellow_player_1.status == "active" &&
      yellow_player_1.current_position + dice_value <=
        yellow_player_1.path.length
    )
      active_players++;
    if (
      yellow_player_2.status == "active" &&
      yellow_player_2.current_position + dice_value <=
        yellow_player_2.path.length
    )
      active_players++;
    if (
      yellow_player_3.status == "active" &&
      yellow_player_3.current_position + dice_value <=
        yellow_player_3.path.length
    )
      active_players++;
    if (
      yellow_player_4.status == "active" &&
      yellow_player_4.current_position + dice_value <=
        yellow_player_4.path.length
    )
      active_players++;
  }
  if (home == ".blue-home") {
    if (
      blue_player_1.status == "active" &&
      blue_player_1.current_position + dice_value <= blue_player_1.path.length
    )
      active_players++;
    if (
      blue_player_2.status == "active" &&
      blue_player_2.current_position + dice_value <= blue_player_2.path.length
    )
      active_players++;
    if (
      blue_player_3.status == "active" &&
      blue_player_3.current_position + dice_value <= blue_player_3.path.length
    )
      active_players++;
    if (
      blue_player_4.status == "active" &&
      blue_player_4.current_position + dice_value <= blue_player_4.path.length
    )
      active_players++;
  }
...
function movePlayer(M_Player, M_Step, M_Road) {
  ...
  if (M_Step == 6 && M_Player.status == "home") {
    ...
    var player_room_update = $(
      M_Player.home + " .white-box .player-room"
    ).html();
    $(M_Player.home + " .white-box .player-room").html(
      player_room_update.replace(M_Player.player, "")
    );
  ...
  if (
    M_Player.current_position + M_Step > M_Player.path.length &&
    getActivePlayers(M_Player.home) == 1
  ) {
  ...
  const moveNow = async () => {
    ...
    for (const item of run_point) {
      ...
      if (M_Player.path[M_Player.current_position] == "win") {
        ...
        winner_sound.play();
        M_Player.status = "win";
        M_Player.previous_step =
          M_Road[M_Player.path[M_Player.current_position - 1]];
        M_Player.previous_step.innerHTML =
          M_Player.previous_step.innerHTML.replace(M_Player.player, "");
        ...
        $(".winner-home " + pl).html(
          $(".winner-home " + pl).html() + M_Player.player
        );
        ...
        if (getWinPlayers(M_Player.home) > 3) {
          if (!current_turn.rank > 0) {
            ...
            console.log(
              M_Player.color + " : You just got " + rank_count + " rank "
            );
          }
          //console.log("winner "+rank_count, current_turn.group);
          $(M_Player.home + " .white-box").css(
            "background-image",
            "url(assets/img/crown" + rank_count + ".png)"
          );
        ...
        if (M_Player.current_position != 0) {
          ...
          M_Player.current_step.innerHTML =
            M_Player.current_step.innerHTML + M_Player.player;
        } else {
          M_Player.current_step.innerHTML =
            M_Player.current_step.innerHTML + M_Player.player;
        }
  ...
  moveNow().then(() => {
    ...
    if (
      kill_zone.children.length > 1 &&
      !safe_stops.includes(M_Player.path[M_Player.current_position - 1])
    ) {
      ...
      for (i = 0; i < kill_zone.children.length; i++) {
        ...
        if (
          kill_zone.children[i] != M_Player.player &&
          enemy.color != M_Player.color
        ) {
...
function activateDice() {
  ...
  $("#dice").html(
    "<img src='assets/img/dice" + dice_value + ".png' class='dice'/>"
  );
...
$("#movegreen1").click(() => {
  ...
  if (
    green_player_1.current_position + dice_value >
    green_player_1.path.length
  ) {
    console.log("dice value is too big for this player, try another player");
    return 0;
  }
...
$("#movegreen2").click(() => {
  ...
  if (
    green_player_2.current_position + dice_value >
    green_player_2.path.length
  ) {
    console.log("dice value is too big for this player, try another player");
    return 0;
  }
...
$("#movegreen3").click(() => {
  ...
  if (
    green_player_3.current_position + dice_value >
    green_player_3.path.length
  ) {
    console.log("dice value is too big for this player, try another player");
    return 0;
  }
...
$("#movegreen4").click(() => {
  ...
  if (
    green_player_4.current_position + dice_value >
    green_player_4.path.length
  ) {
    console.log("dice value is too big for this player, try another player");
    return 0;
  }
